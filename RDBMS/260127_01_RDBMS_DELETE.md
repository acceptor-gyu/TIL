# 데이터베이스 삭제 메커니즘 DELETE와 VACUUM (Tombstone)

## 궁금증의 시작 (즉시 삭제 시 비효율성에 대해)
즉시 삭제의 문제
- row 삭제 → 페이지 재배치
- 인덱스 노드 병합
- 파일 크기 조정

=> 작은 DELETE 하나에 디스크 대량 쓰기 발생

위와 같은 생각을 시작으로 RDBMS에서 DELETE 명령 시 어떻게 동작하는지 궁금했습니다.
...
일반적인 배열(array)에서 중간의 데이터를 찾아 삭제하고 삭제한 데이터 뒤부터 전부 앞으로 당기는 것을 생각하면 이해하기 쉬울 것같습니다.
...
| 항목 | 개인 프로젝트                  | MySQL InnoDB |
|------|--------------------------|--------------|
| 실행 방식 | 수동 `VACUUM` 명령 또는 주기적 실행 | 자동 (Purge Thread) |
