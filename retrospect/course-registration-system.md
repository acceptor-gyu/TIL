...
4. 토큰이 생각보다 더 빨리 소진되어 test-planner sub agent의 model을 sonnet으로 변경한 것
   - Opus (계획/설계): 요구사항 분석, 아키텍처 설계, 작업 분해 등은 깊은 추론이 필요하고 토큰 사용량이 상대적으로 적다. 여기서 판단을 잘못하면 구현 전체가 틀어지므로 비싼 모델을 쓸 가치가 있다. 
   - Sonnet (구현): 실제 코드 작성, 리팩토링, 테스트 작성 등은 반복적이고 토큰 소모가 크다. Sonnet이 코딩 능력은 충분하면서 비용이 훨씬 낮으므로 가성비가 좋다.
