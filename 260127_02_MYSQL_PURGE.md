...
### 1.5 MySQL Purge 설정 및 튜닝

InnoDB는 여러 시스템 변수로 Purge 동작을 제어합니다.

#### 1.5.1 innodb_purge_threads

Purge를 수행하는 백그라운드 스레드 개수입니다.
...
**튜닝 가이드:**
```sql
-- DML이 소수의 테이블에 집중된 경우
SET GLOBAL innodb_purge_threads = 1;  -- 경합 감소

-- DML이 여러 테이블에 분산된 경우
SET GLOBAL innodb_purge_threads = 8;  -- 병렬 처리
```

#### 1.5.2 innodb_max_purge_lag

Purge가 지연될 때(History List가 길어질 때) INSERT/UPDATE/DELETE 작업에 지연을 부과하여 Purge가 따라잡을 시간을 줍니다.
...
**지연 계산 공식:**
```
delay(microseconds) = (purge_lag / innodb_max_purge_lag - 0.9995) × 10000
```
...
delay = (1,500,000 / 1,000,000 - 0.9995) × 10000
      = (1.5 - 0.9995) × 10000
      = 5,005 microseconds (약 5ms)
```

#### 1.5.3 innodb_purge_batch_size

한 번의 Purge 배치에서 처리할 undo log 페이지 수입니다.
...
**동작:**
- 멀티스레드 환경: 배치 크기가 `innodb_purge_threads`로 나뉨
- 4개 스레드 × 300 배치 = 각 스레드당 75 페이지 처리
- 128회 반복마다 더 이상 필요 없는 undo log 페이지를 해제

#### 1.5.4 innodb_max_purge_lag_delay

`innodb_max_purge_lag`로 인한 최대 지연 시간(마이크로초)을 제한합니다.

```sql
-- DML이 과도하게 차단되는 것을 방지
SET GLOBAL innodb_max_purge_lag_delay = 200000;  -- 최대 200ms
```
...
innodb_max_purge_lag_delay = 200000
...
#### 4.4.1 SHOW ENGINE INNODB STATUS

MySQL에서 Purge 상태를 확인하는 가장 기본적인 방법입니다.

```sql
SHOW ENGINE INNODB STATUS\G
```

**출력 예시 (TRANSACTIONS 섹션):**
```
------------
TRANSACTIONS
------------
...
| 지표 | 의미 | 정상 범위 |
|------|------|-----------|
| `Trx id counter` | 현재 트랜잭션 ID | - |
| `Purge done for trx's n:o <` | Purge가 완료된 트랜잭션 ID | - |
| `History list length` | 아직 Purge되지 않은 트랜잭션 수 (Purge Lag) | < 1000 |

**History List Length 해석:**

```sql
-- 정상: History list length가 낮음
History list length 20           -- 정상 (Purge가 잘 따라가고 있음)

-- 경고: History list length가 높음
History list length 50000        -- 경고 (Purge가 지연되고 있음)

-- 위험: History list length가 매우 높음
History list length 500000       -- 위험 (Purge가 심각하게 지연)
```

#### 4.4.2 Purge Lag 발생 원인
...
-- 문제 상황
START TRANSACTION;
SELECT * FROM large_table;  -- 10분 동안 실행
-- COMMIT/ROLLBACK을 하지 않음
-- → Purge가 이 트랜잭션이 끝날 때까지 대기
```
...
```bash
# mysqldump가 실행되는 동안 Purge가 차단됨
mysqldump --single-transaction --all-databases > backup.sql
# → History list length가 급증
```

**해결책:**
- 트래픽이 적은 시간대에 백업 실행
- `innodb_max_purge_lag`로 DML 지연 허용

**3. 대량 DELETE 후 Purge 지연**

```sql
-- 100만 행 삭제
DELETE FROM logs WHERE created_at < '2024-01-01';
-- → Purge가 100만 행을 처리하는 데 시간 소요
```
...
-- 10000행씩 여러 번 실행 (Purge가 따라갈 시간 제공)
```
...
```bash
#!/bin/bash
# purge_monitor.sh - Purge Lag 모니터링
...
while true; do
    ...
    # History list length 추출
    mysql -u root -p'password' -e "SHOW ENGINE INNODB STATUS\G" | grep "History list length"
...
**출력 예시:**
```
2026-01-28 10:15:00  12777  3
History list length 45

2026-01-28 10:15:05  13105  2
History list length 52

2026-01-28 10:15:10  13890  1
History list length 38
```

#### 4.4.4 프로메테우스/그라파나 모니터링

**MySQL Exporter로 메트릭 수집:**
...
**쿼리 예시:**
```promql
# History list length 추이
mysql_global_status_innodb_purge_trx_id_age

# Purge 처리량 (초당 처리된 행 수)
rate(mysql_global_status_innodb_rows_purged[1m])

# 활성 트랜잭션 수
mysql_info_schema_innodb_trx

# Purge Lag 경고 (1000 초과 시)
mysql_global_status_innodb_purge_trx_id_age > 1000
```
...
**1. Purge 스레드 증가**
```sql
-- DML이 여러 테이블에 분산되어 있다면
SET GLOBAL innodb_purge_threads = 8;
```

**2. Purge Lag 제어**
```sql
-- Purge가 지연될 때 DML 속도 제한
SET GLOBAL innodb_max_purge_lag = 1000000;
SET GLOBAL innodb_max_purge_lag_delay = 100000;  -- 최대 100ms 지연
```

**3. 오래된 트랜잭션 방지**
```sql
-- 애플리케이션에서 autocommit 활성화
SET autocommit = 1;
...
```sql
-- 안 좋은 예: 한 번에 대량 삭제
DELETE FROM logs WHERE created_at < '2024-01-01';  -- 100만 행
...
개인 프로젝트의 VACUUM 구현은 **교육 목적으로 최적화**되어 있으며, MySQL의 복잡성 없이도 **핵심 개념을 이해**할 수 있습니다.
...
- [MySQL 8.4 Reference Manual - InnoDB Startup Options and System Variables](https://dev.mysql.com/doc/refman/8.4/en/innodb-parameters.html)
  - 모든 InnoDB 시스템 변수 레퍼런스
  - `innodb_purge_*` 관련 변수 상세 설명
...
- [MySQL 8.0 Reference Manual - InnoDB Architecture](https://dev.mysql.com/doc/refman/8.0/en/innodb-architecture.html)
  ...
  - Undo Log와 Redo Log 구조
  - B-Tree 인덱스와 Clustered Index

- [AWS - Achieve a high-speed InnoDB purge on Amazon RDS for MySQL and Amazon Aurora MySQL](https://aws.amazon.com/blogs/database/achieve-a-high-speed-innodb-purge-on-amazon-rds-for-mysql-and-amazon-aurora-mysql/)
  - 실무에서 Purge Lag 문제 해결 사례
  - RDS/Aurora 환경에서의 Purge 최적화

### PostgreSQL VACUUM (비교)
- [PostgreSQL Documentation - VACUUM](https://www.postgresql.org/docs/current/sql-vacuum.html)
  - PostgreSQL의 VACUUM 명령
  - `VACUUM`, `VACUUM FULL`, `VACUUM ANALYZE` 차이
  - Autovacuum 설정
...
### 커뮤니티 자료
- [Percona Blog - Understanding InnoDB Purge and History List](https://www.percona.com/blog/understanding-innodb-purge-and-history-list/)
  - Purge와 History List 심층 분석
  - 실전 트러블슈팅 가이드

- [MariaDB Documentation - InnoDB Purge](https://mariadb.com/kb/en/innodb-purge/)
  - MariaDB의 InnoDB Purge 구현
  - MySQL과의 차이점

### 추천 튜닝 도구
- [Releem - Best Practices of tuning MySQL](https://releem.com/docs/mysql-performance-tuning/innodb_purge_threads)
  - `innodb_purge_threads` 튜닝 가이드
  - 실시간 모니터링 및 자동 튜닝 도구

- [MySQL Shell - InnoDB Cluster and Binary Log Purging](https://dev.mysql.com/doc/mysql-shell/9.5/en/innodb-cluster-binary-log-purging.html)
  - InnoDB Cluster 환경에서의 Binary Log와 Purge 관리

    
**학습 가치:**
- MVCC와 트랜잭션 격리의 중요성
- Copy-on-Write 패턴을 통한 동시성 제어
- 실무에서의 Trade-off 결정 (단순함 vs 성능)
