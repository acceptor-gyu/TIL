...
#### Covering Index가 이렇게 큰 성능 차이를 만드는 이유

파티셔닝만 적용했을 때(5.268s)와 Covering Index까지 적용했을 때(2.604s)의 차이는 약 2배다. 단순히 "인덱스를 탔다"는 것만으로는 이 차이를 설명하기 어렵다. 근본적인 원인은 **I/O 접근 방식의 변화**에 있다.

**1. Row Lookup(Random I/O) 제거**

파티셔닝만 적용된 `SELECT *` 쿼리는 다음과 같은 경로를 거친다:

1. Partition Pruning으로 해당 월 파티션을 선택
2. 파티션 내 클러스터드 인덱스(데이터 페이지)를 풀 스캔
3. 모든 컬럼 데이터를 읽어 반환

반면 Covering Index를 적용한 `SELECT timestamp, ticker` 쿼리는:

1. Partition Pruning으로 해당 월 파티션을 선택
2. `idx_timestamp_ticker` 세컨더리 인덱스만 스캔
3. **데이터 페이지에 접근하지 않고** 인덱스 리프 노드에서 바로 결과 반환

즉, 클러스터드 인덱스(데이터 페이지)로의 **Row Lookup이 완전히 사라진다**. 이 Row Lookup은 인덱스의 정렬 순서와 데이터의 물리적 위치가 다를 때 Random I/O를 유발하는 대표적인 원인이다.

**2. 읽어야 할 페이지 수의 차이**

InnoDB는 데이터를 16KB 페이지 단위로 관리한다. 이 테이블의 한 행에는 `id(8B)`, `timestamp(8B)`, `ticker(~255B)`, `open/high/low/close/volume(각 8B)`, `source/unit(~255B)` 등 약 **800B 이상**의 데이터가 담긴다.

- **데이터 페이지**: 한 페이지에 약 20행 저장 (16KB / 800B)
- **인덱스 페이지** (`timestamp` + `ticker`만 포함): 한 페이지에 약 **50~60행** 저장 (16KB / ~270B)

5일치 데이터가 약 100만 건이라고 가정하면:
...
읽어야 할 페이지 수 자체가 **약 2.8배 적다**. 이는 디스크 I/O뿐 아니라 Buffer Pool 내에서의 메모리 접근에서도 동일하게 효과가 나타난다.

**3. Buffer Pool 효율 극대화**

인덱스 페이지는 데이터 페이지보다 크기가 작으므로, 동일한 Buffer Pool 공간에 **더 많은 인덱스 페이지를 캐싱**할 수 있다. 이는 Buffer Pool 히트율을 높여 디스크 I/O 빈도를 추가로 감소시킨다. 특히 Docker 환경처럼 메모리가 제한된 상황에서는 이 차이가 더욱 두드러진다.

> 정리하면, Covering Index의 성능 개선은 단순히 "인덱스를 사용해서"가 아니라, **Row Lookup Random I/O 제거 + 읽어야 할 페이지 수 감소 + Buffer Pool 캐싱 효율 증가**라는 세 가지 요인이 복합적으로 작용한 결과다.

## R (Result) — 어떤 자료로 검증했는가?
