...
#### 1. minimumIdle (최소 유휴 커넥션 수)
- 풀에 항상 유지할 최소 유휴 커넥션 수
- 기본값: maximumPoolSize와 동일
- 권장: 평균 동시 요청 수

#### 2. maximumPoolSize (최대 풀 크기)
- 풀이 유지할 수 있는 최대 커넥션 수
- 기본값: 10
- 권장: `connections = (core_count * 2) + effective_spindle_count`

#### 3. connectionTimeout (커넥션 획득 타임아웃)
- 풀에서 커넥션을 얻기 위해 대기하는 최대 시간
- 기본값: 30초
- 권장: 5~10초

#### 4. maxLifetime (커넥션 최대 수명)
- 커넥션이 풀에서 유지되는 최대 시간
- 기본값: 30분
- 권장: DB의 wait_timeout보다 짧게 설정

#### 5. idleTimeout (유휴 커넥션 타임아웃)
- 사용하지 않는 커넥션을 풀에서 제거하기까지의 시간
- 기본값: 10분
- 권장: 5~10분
